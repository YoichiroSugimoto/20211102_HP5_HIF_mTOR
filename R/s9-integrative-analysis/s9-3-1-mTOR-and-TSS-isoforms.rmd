...
## To be deleted
p.dt <- fread(file.path(s8.2.dte.iso.dir, "RCC4_VHL_EIF4E2_NA.csv"))
...
## To be deleted
d.sl.sig.rcc4.vhl.mrl.dt <- merge(
    d.sl.sig.rcc4.vhl.mrl.dt,
    p.dt[!duplicated(gene_id), .(gene_id, gene_FDR)],
    by = "gene_id"
)

d.sl.sig.rcc4.vhl.mrl.dt[
    gene_FDR < 0.001 & te_diff_by_tss == "Large (> 1.5)"
][, .(gene_name,
      tss_name_Hypersensitive, tss_name_Resistant,
      TOP_motif_length_Hypersensitive, TOP_motif_length_Resistant, uORF_all_Hypersensitive, uORF_all_Resistant)][
    uORF_all_Hypersensitive == 0 & uORF_all_Resistant > 2
]
...
tx.meta.info.dt[
    gene_id == "ENSG00000023697",
    .(tss_name, tss_p1_TOP, uORF_all)
]
...
r4vhl.prop.dt <- r4vhl.prop.dt[
    tss_name %in% sl.sig.rcc4.vhl.mrl.dt[, tss_name]
]
