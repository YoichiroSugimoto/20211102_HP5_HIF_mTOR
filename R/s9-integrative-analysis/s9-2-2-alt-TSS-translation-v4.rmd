...
```{r plot te differences, fig.width = 7, fig.height = 20}
...
g2 <- ggplot(
    ...
    geom_text(
        aes(
            label = if_else(
                base_vs_alternate_CDS == "Different", "Yes", NA_character_,
                missing = "(NA)"
            )
        )
    ) +
...
UTR5_len = ifelse(d_utr5_len > 0, "longer", "shorter")
