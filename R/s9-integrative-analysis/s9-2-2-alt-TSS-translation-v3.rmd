...
{Reduce(
     ...
     list(filtered.cluster.with.quantile.dt[, .(tss_name, q50)], tss.startcodon.dt, .)
 )}

tx.meta.dt[, TOP_motif_length := tss_p1_pTOP + tss_p2_pTOP + tss_p3_pTOP]
...
d.alt.vs.base.dt <- dcast(
    ...
    value.var = c("MRL_treated", meta.cols)
)
...
d.alt.vs.base.dt[, `:=`(
    ...
    CDS_different = case_when(
        strand == "+" ~ 
    )
)]

d.alt.vs.base.dt[, table(ifelse(MRL_log2fc > 0, "Up", "Down"), d_uORF > 0)] %>%
    fisher.test 
