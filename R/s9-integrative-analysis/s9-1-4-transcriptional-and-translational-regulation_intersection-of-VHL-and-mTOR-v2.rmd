...
tx.meta.info.dt <- file.path(s8.3.dir, "processed-tx-meta-info.csv") %>% fread
...
tx.meta.info.dt <- merge(
    ...
    merge(
        y = th2.all.de.dte.res.dt[
            mRNA_trsl_intersection_by_functions != "Others"
        ][, .(gene_id, gene_name, mRNA_trsl_intersection_by_functions)],
        by = "gene_id"
    )
...
print("Before filtration by feature assignment")
tx.meta.info.by.class.dt[, table(mRNA_trsl_intersection_by_functions)]

print("After filtration by feature assignment")
tx.meta.info.by.class.dt[
    total_proportion > 0.5
][, table(mRNA_trsl_intersection_by_functions)]

## uORF number
tx.meta.info.by.class.dt[total_proportion > 0.5] %$%
    pairwise.wilcox.test(x = uORF_all, g = mRNA_trsl_intersection_by_functions)

ggplot(
    data = tx.meta.info.by.class.dt[total_proportion > 0.5],
    aes(
        x = gsub("or", "or\n", mRNA_trsl_intersection_by_functions),
        y = uORF_all
    )
...
## CDS length
tx.meta.info.by.class.dt[total_proportion > 0.5] %$%
    pairwise.wilcox.test(x = cds_len, g = mRNA_trsl_intersection_by_functions)
...
count.prop.dt <- tx.meta.info.by.class.dt[
    total_proportion > 0.5
][, .N, by = list(mRNA_trsl_intersection_by_functions, cds_len_bin)]
count.prop.dt[, gene_sum := sum(N), by = mRNA_trsl_intersection_by_functions]
count.prop.dt[, ratio := N/gene_sum]
